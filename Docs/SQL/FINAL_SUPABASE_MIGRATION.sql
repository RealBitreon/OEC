-- ============================================================================
-- FINAL COMPLETE SUPABASE MIGRATION
-- This is the ONLY SQL file you need to run
-- Converts entire application from JSON to Supabase PostgreSQL
-- ============================================================================
-- 
-- INSTRUCTIONS:
...
CREATE TABLE users (
    ...
    email TEXT,
    role TEXT NOT NULL CHECK (role IN ('CEO', 'LRC_MANAGER', 'VIEWER')) DEFAULT 'VIEWER',
...
-- ============================================================================
-- TABLE 4: QUESTIONS
-- Stores questions for competitions and training
-- ============================================================================
CREATE TABLE questions (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    competition_id UUID REFERENCES competitions(id) ON DELETE CASCADE,
    is_training BOOLEAN DEFAULT false,
    type TEXT NOT NULL DEFAULT 'mcq' CHECK (type IN ('mcq', 'true_false', 'short_answer', 'text', 'fill_blank', 'essay')),
    category TEXT,
    difficulty TEXT,
...
-- ============================================================================
-- TABLE 5: SUBMISSIONS
-- Stores participant submissions
-- ============================================================================
...
-- ============================================================================
-- TABLE 6: WHEEL PRIZES
-- Stores prizes for the wheel system
-- ============================================================================
...
-- Sessions policies
CREATE POLICY "Anyone can manage sessions" ON sessions FOR ALL USING (true);

-- Competitions policies
CREATE POLICY "Anyone can view competitions" ON competitions FOR SELECT USING (true);
CREATE POLICY "Service role can manage competitions" ON competitions FOR ALL USING (true);
...
CREATE POLICY "Service role can manage wheel prizes" ON wheel_prizes FOR ALL USING (true);

-- Wheel spins policies
CREATE POLICY "Anyone can view wheel spins" ON wheel_spins FOR SELECT USING (true);
CREATE POLICY "Service role can manage wheel spins" ON wheel_spins FOR ALL USING (true);
...
BEGIN
    ...
    IF v_rules->>'eligibilityMode' = 'all_correct' THEN
        IF v_score = v_total THEN
            v_tickets := COALESCE((v_rules->>'ticketsPerCorrect')::INTEGER, 1) * v_score;
        END IF;
    ELSIF v_rules->>'eligibilityMode' = 'min_correct' THEN
        IF v_score >= COALESCE((v_rules->>'minCorrectAnswers')::INTEGER, 0) THEN
            v_tickets := COALESCE((v_rules->>'ticketsPerCorrect')::INTEGER, 1) * v_score;
        END IF;
    ELSE
        v_tickets := COALESCE((v_rules->>'ticketsPerCorrect')::INTEGER, 1) * v_score;
...
-- Insert Users (from your JSON data)
...
BEGIN
    ...
    RAISE NOTICE '  1. users (3 records)';
    RAISE NOTICE '  2. sessions (1 record)';
    RAISE NOTICE '  3. competitions (1 record)';
    RAISE NOTICE '  4. questions (5 records)';
    RAISE NOTICE '  5. submissions (3 records)';
    RAISE NOTICE '  6. wheel_prizes (0 records)';
    RAISE NOTICE '  7. wheel_spins (0 records)';
    RAISE NOTICE '  8. audit_logs (0 records)';
    RAISE NOTICE '============================================================================';
    RAISE NOTICE 'Features enabled:';
    RAISE NOTICE '  ✓ Row Level Security (RLS)';
    RAISE NOTICE '  ✓ Automatic timestamps';
    RAISE NOTICE '  ✓ Helper functions';
    RAISE NOTICE '  ✓ Useful views';
    RAISE NOTICE '  ✓ Optimized indexes';
    RAISE NOTICE '============================================================================';
    RAISE NOTICE 'Next steps:';
    RAISE NOTICE '  1. Set environment variables in .env';
    RAISE NOTICE '  2. Run: npm run dev';
    RAISE NOTICE '  3. Test login with existing users';
    RAISE NOTICE '  4. Deploy to production';
    RAISE NOTICE '============================================================================';
    RAISE NOTICE '🎉 YOUR APPLICATION IS NOW POWERED BY SUPABASE!';
    RAISE NOTICE '============================================================================';
END $$;
